...
export const StatusOfferController = ({ id, isActive, activeOffer, token }) => {
  const handleClick = async () => {
    ...
    const confirmationMessage = isActive
      ? `¿Quieres desactivar la oferta?`
      : `¿Quieres activar la oferta?`;
    ...
    if (confirmed.isConfirmed) {
      try {
        ...
        const response = await fetch(`${URL}/offers/toggleActivation/${id}`, {
          method: "PUT",
          headers: {
        ...
        if (response.ok) {
          ...
          activeOffer(id, responseData.isActive);
        } else {
          Swal.fire({
            icon: "error",
